...
WR0:
	...
	DB	':'
	LXI	B,16		; INITIALIZE B := 0, C := AH (DECIMAL 16)
;-------------------------------
	PUSH	H		; SAVE HL
WR1:
...
CI4:
	...
	JMP	@USER
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 'BREAK' - ENTERED VIA CALLS FROM 'BLK','COM','LDM' ROUTINES
...
RI7:				; THE PAPER TAPE READER IS READY
	...
	POP	B		; RESTORE BC
	POP	H
	RET			; RETURN SUCCESSFULLY WITH CARRY = 0
;-------------------------------
...
CRTOT1:				; INTELLEC WITH SERIALLY CONNECTED CRT
	...
	MOV	A,C		; MOVE CHARACTER TO BE OUTPUT TO C-REG
...
LO:				; LIST OUTPUT
	...
	MVI	A,L1LOC AND 0FFH; A := LSB OF L1LOC ADDRESS
...
CS0:
	...
	POP	H		; RESTORE H,L
	RRC			; ROTATE BIT 0 INTO CARRY; THUS CARRY = 1
...
BYTE:
	...
	CALL	NIBBLE		; CONVERT OT 4 BIT HEX; NOW LSB OF A-REG
...
CONV:
	...
	DAA
	MOV	C,A		; STORE CONVERTED RESULT IN C-REG
	RET			; RETURN
;////////////////////////////////////////////////////////////////////////////
...
PA1:
	...
	CALL	NIBBLE		; CONVERT THE ASCII CHARACTER TO HEX; MUST BE
...
;/          (PUSHED ON STACK AT TIME OF RST 0 INSTRUCTION OR WHEN CONSOLE       /
...
RESTART:
	...
	MVI	A,NOT INT0	; SET MONTIOR'S DEFAULT INTERRUPT MASK
...
RST0:				; MOVE THE MACHINE STATE FROM THE USER'S STACK
				...
				;------------!--------!--------!------!------
	DCX	H		;            !        !        !      !
	MOV	M,D		;SLOC=MSB(SP)!ALOC=A  !ILOC=INT!BLOC=B!DLOC=D
	DCX	H		;            !        !        !      !
...
;            ENTERED VIA CALLS FROM 'IOCDR1' AND 'IOCDR2'
...
; 'PIODR2' - ENTERED VIA CALLS FROM 'UI','UPPS' ROUTINES
...
PIODR2:
	...
	RET
;-----------------------------------------------------------------------
; 'PIODR3' - ENTERED VIA CALLS FROM 'POC','PO','LO','UI','UO' ROUTINES
